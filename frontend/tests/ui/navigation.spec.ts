...
test.describe('Responsive Navigation', () => {
  test.describe('Mobile Bottom Tabs (< 768px)', () => {
    ...
    test('should display bottom tabs on mobile', async ({ page }) => {
      ...
      await expect(navLinks.nth(3)).toContainText('Setups');
      await expect(navLinks.nth(4)).toContainText('Agents');
      await expect(navLinks.nth(5)).toContainText('Lists');
    });
  ...
  test.describe('Active State Matching', () => {
    ...
    test('should match nested routes correctly', async ({ page }) => {
      ...
      // Lists tab should be active (index 5 - after Analysis, Live 20, Arena, Setups, Agents)
      const listsTab = page.locator('nav[aria-label="Mobile navigation"] a').nth(5);
