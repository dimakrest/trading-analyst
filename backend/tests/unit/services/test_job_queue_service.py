...
class TestJobQueueServiceInitialization:
    ...
    def test_init_with_custom_worker_id(self, test_session_factory):
        """Should use provided worker ID."""
        service = JobQueueService(
            test_session_factory, Live20Run, worker_id="custom-worker-123"
        )
        assert service.worker_id == "custom-worker-123"

    def test_init_generates_worker_id(self, test_session_factory):
    ...
    def test_init_with_worker_type(self, test_session_factory):
        """Should generate worker ID with custom type prefix."""
        service = JobQueueService(
            test_session_factory, Live20Run, worker_type="live20"
        )
        assert service.worker_id.startswith("live20-")
        assert len(service.worker_id) == len("live20-") + 8

    def test_init_worker_id_takes_precedence_over_worker_type(self, test_session_factory):
    ...
    def test_init_stores_model_class(self, test_session_factory):
...
class TestClaimNextJob:
    ...
    async def test_claim_next_job_respects_fifo_order(
        ...
        run1 = await create_live20_run(test_session_factory, status="pending")
        run2 = await create_live20_run(test_session_factory, status="pending")
        run3 = await create_live20_run(test_session_factory, status="pending")
    ...
    async def test_claim_next_job_skips_non_pending(
        ...
        await create_live20_run(test_session_factory, status="running")
        await create_live20_run(test_session_factory, status="completed")
        await create_live20_run(test_session_factory, status="failed")
        pending_run = await create_live20_run(test_session_factory, status="pending")
...
class TestIsCancelled:
    ...
    async def test_is_cancelled_returns_true_for_cancelled(
        ...
        run = await create_live20_run(test_session_factory, status="cancelled")
        service = JobQueueService(test_session_factory, Live20Run)
...
class TestResetStrandedJobs:
    ...
    async def test_reset_stranded_jobs_resets_running_to_pending(
        ...
        run2 = await create_live20_run(test_session_factory, status="running")
        ...
        updated1 = await get_run_by_id(test_session_factory, run1.id)
        updated2 = await get_run_by_id(test_session_factory, run2.id)
    ...
    async def test_reset_stranded_jobs_does_not_increment_retry_count(
        ...
        run = await create_live20_run(
            test_session_factory, status="running", retry_count=1
        )
    ...
    async def test_reset_stranded_jobs_returns_zero_when_none(
        ...
        await create_live20_run(test_session_factory, status="pending")
        await create_live20_run(test_session_factory, status="completed")
...
class TestUpdateHeartbeat:
    ...
    async def test_update_heartbeat_updates_timestamp(
        ...
        run = await create_live20_run(
            test_session_factory, status="running", heartbeat_at=old_time
        )
...
class TestMarkFailed:
    ...
    async def test_mark_failed_increments_retry_count(
        ...
        run = await create_live20_run(
            test_session_factory, status="running", retry_count=0, max_retries=3
        )
        service = JobQueueService(test_session_factory, Live20Run)
