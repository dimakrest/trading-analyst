...
class TestAnalyzeRSI2:
    ...
    def test_rsi2_strong_overbought_85_to_95(self):
        ...
        # Verify long score matches value
        assert result.long_score == _score_for_long(result.value)
    ...
    def test_rsi2_boundary_values(self):
        """Test RSI-2 LONG scoring at exact boundary values."""
        # Test exact boundaries for LONG scoring (5, 15, 30, 50)
        boundaries = [
            (5.0, 15),   # Exactly 5: LONG 15pts
            (15.0, 10),  # Exactly 15: LONG 10pts
            (30.0, 5),   # Exactly 30: LONG 5pts
            (50.0, 0),   # Exactly 50: 0pts (not oversold)
            (70.0, 0),   # Exactly 70: 0pts (not oversold)
            (95.0, 0),   # Exactly 95: 0pts (not oversold)
        ]

        for value, expected_long in boundaries:
            assert _score_for_long(value) == expected_long, f"LONG score mismatch at RSI={value}"

    def test_rsi2_long_score_populated(self):
        """Test that long_score is always populated."""
        closes = [100.0] * 5 + [95.0, 90.0, 85.0, 80.0]

        result = analyze_rsi2(closes)

        # long_score should be int, not None
    ...
    def test_rsi2_consistency_with_scoring_functions(self):
        """Test that RSI2Analysis long_score matches standalone scoring function."""
        closes = [100.0] * 5 + [95.0, 92.0, 89.0, 86.0, 83.0]

        result = analyze_rsi2(closes)

        # Score should match scoring function
        assert result.long_score == _score_for_long(result.value)
